"""Fetch and parse Liberal Institute articles into structured JSON."""
...
def parse_article(url: str, timeout: int = 30) -> LibInstArticle:
    ...
    return LibInstArticle(
        ...
        publisher_name="Liberal Institute",
...
def fetch_article_links(limit: int = 20, timeout: int = 20) -> List[str]:
    """Discover Liberal Institute publication/article URLs from index pages."""
    seeds = [
        "https://www.libinst.ch/denkanstoesse/",
        "https://www.libinst.ch/fr/publications/",
    ]
    seen: set[str] = set()
    links: List[str] = []
    ...
    for seed in seeds:
        ...
        for a in soup.select("a[href]"):
            ...
            if "libinst.ch" not in full:
                continue
            if not ("/denkanstoesse/" in full or "/publications/" in full):
                continue
            if full.rstrip("/") == seed.rstrip("/"):
                continue
            full = full.split("?", 1)[0].rstrip("/")
            if not re.search(r"/[a-z0-9][a-z0-9-]{8,}$", full):
                continue
            if full in seen:
                continue
            seen.add(full)
...
def _make_filename(article: dict) -> str:
    return stable_article_filename(article, "lib_inst")


def scrape_lib_inst(urls: List[str], out_dir: Path, timeout: int = 30) -> None:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Scrape Liberal Institute articles into JSON files.")
    parser.add_argument("--limit", type=int, default=20, help="Max articles to discover.")
    parser.add_argument("--timeout", type=int, default=30)
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "data" / "webdata_lib_inst",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Plain-text file with one URL per line. If omitted, discover from Lib Inst index pages.",
    )
    ...
    if args.urls_file:
        ...
        print("Fetching article links from Lib Inst pages...")
        urls = fetch_article_links(limit=args.limit, timeout=args.timeout)
        print(f"Found {len(urls)} article links.")

    scrape_lib_inst(urls, args.out_dir, timeout=args.timeout)
